package com.scanner.project; // MUST BE PRESENT
...
public class ConcreteSyntax {
	...
	private Expression relation() {
		...
		while (token.getValue().equals("<") || token.getValue().equals("<=")
			...
			b.term1 = e;
	        b.op = new Operator(token.getValue());
	        token = input.nextToken(); // consume Operator
	        b.term2 = addition();
			e = b;
		}
		return e;
	...
	private Expression addition() {
		...
		while (token.getValue().equals("+") || token.getValue().equals("-")) {
	        ...
	        b.term1 = e;
	        b.op = new Operator(token.getValue());
	        token = input.nextToken(); // consume + or -
	        b.term2 = term();
	        e = b;
		}
		return e;
	...
	private Expression term() {
		...
		while (token.getValue().equals("*") || token.getValue().equals("/") || token.getValue().equals("%")) {
			...
			b.term1 = e;
	        b.op = new Operator(token.getValue());
	        token = input.nextToken(); // consume * or / or %
	        b.term2 = negation();
			e = b;
		}
		return e;
